...
class RotaryEmbedding(nn.Module):
    ...
    #equivelant forward but without using einsum    
    def forward(self, t, cache_key=None):
        should_cache = not self.learned_freq and cache_key is not None
        ...
        # Replace einsum('..., f -> ... f', t, freqs) with explicit broadcasting
        # The operation expands dimensions to perform element-wise multiplication
        freqs = t.unsqueeze(-1) * freqs  # This is the equivalent to `einsum('..., f -> ... f', t, freqs)`

        # Duplicate frequencies along the last dimension to simulate the repeat in `einsum`
    ...
    # original forward implementation, left here for reference.
    def forward_old(self, t, cache_key = None):
